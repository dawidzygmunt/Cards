...
import SingleCollection from './components/displayCollections'
    ...
    const getEdition = async () => {
      ...
      collectionsIds.forEach(async (colectionId: string) => {
        try {
          ...
          const response = await axios.get(`/api/v1/collections/${colectionId}`)
      ...
      <div className='flex m-10 flex-wrap'>
        ...
        {data && data.map((item) => (
          <Link key={item._id} to={`/admin/editions/${item._id}`}>
            <SingleCollection data={item} key={item._id} />
          </Link>
        ))}
